...
where dbid in (db_id('MD360Gate'),db_id('MD360CommonDB'),db_id('MD360GlobalDB'),db_id('MD360LabDB'),db_id('AlamedaDB'),db_id('HarrisDB'),db_id('ParklandDB'),db_id('AlamedaDoctorDB'),db_id('ParklandDoctorDB'),db_id('HarrisNurseDB'),db_id('ParklandNurseDB')) and cmd = 'AWAITING COMMAND' and spid <> @@spid;
exec(@sql);
end;
go
...
if db_id('MD360LabDB') 			is not null drop database MD360LabDB;
...
exec PdbcreatePartition 'MD360Gate','MD360LabDB',@DatabaseTypeId=4,@StandAloneSchemaName='Lab';
